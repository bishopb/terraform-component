...
#
before_terraform_init() {
  # if this is the first deploy, we need to use local state
  if [ -n "${FIRST_DEPLOY:-}" ]; then
    export original_TF_VAR_backend=${TF_VAR_backend:-}
    unset TF_VAR_backend
  fi
...
#
after_terraform_apply() {
  # if the apply succeeded, and if this is the first deploy, then we need to
  # run init again with the common backend to migrate from local to common
  # state storage
  if [ 0 -eq "$1" -a -n "${FIRST_DEPLOY:-}" ]; then
    export TF_VAR_backend=${original_TF_VAR_backend}
    unset original_TF_VAR_backend
